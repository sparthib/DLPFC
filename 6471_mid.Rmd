...
```{r convert colData to df, echo = FALSE, message = FALSE, results='hide'}
...
```{r counts data, message = FALSE, results='hide'}
...
is_mito <- grepl("(^MT-)|(^mt-)", rowData(spe_6471_mid)$symbol)
table(is_mito)
```
###  list MT genes
``` {r list MT genes, echo = FALSE}
rowData(spe_6471_mid)$symbol[is_mito]
```
### Calculate per-spot QC metrics and store in colData
```{r Calculate per-spot QC metrics and store in colData, echo = FALSE}
spe_6471_mid <- addPerCellQC(spe_6471_mid, subsets = list(mito = is_mito))
```

### library size histogram
```{r library size histogram, echo = FALSE}
hist(colData(spe_6471_mid)$sum, breaks = 40)
```
Spike at low library size?  

### plot cell counts
``` {r plot cell counts, echo = FALSE}
plotQC(spe_6471_mid, type = "scatter", 
...
qc_lib_size_2000 <- colData(spe_6471_mid)$sum < 2000
colData(spe_6471_mid)$qc_lib_size_2000 <- qc_lib_size_2000
plotQC(spe_6471_mid, type = "spots", 
       x_coord = "pxl_row_in_fullres",
       y_coord = "pxl_col_in_fullres",
       discard = "qc_lib_size_2000")
```
### spatial pattern of discarded spots for low threshold 

```{r spatial pattern of disc spots for low threshold}
qc_lib_size_500 <- colData(spe_6471_mid)$sum < 500
colData(spe_6471_mid)$qc_lib_size_500 <- qc_lib_size_500
plotQC(spe_6471_mid, type = "spots", 
       x_coord = "pxl_row_in_fullres",
       y_coord = "pxl_col_in_fullres",
       discard = "qc_lib_size_500")
```
###expressed genes
```{r expressed genes }
#histogram of numbers of expressed genes
hist(colData(spe_6471_mid)$detected, breaks = 20)
```
...
qc_detected <- colData(spe_6471_mid)$detected < 250
colData(spe_6471_mid)$qc_detected <- qc_detected
plotQC(spe_6471_mid, type = "spots", 
       x_coord = "pxl_row_in_fullres",
       y_coord = "pxl_col_in_fullres",
       discard = "qc_detected")
...
qc_detected_1000 <- colData(spe_6471_mid)$detected < 1000
colData(spe_6471_mid)$qc_detected_1000 <- qc_detected_1000
plotQC(spe_6471_mid, type = "spots", 
       x_coord = "pxl_row_in_fullres",
       y_coord = "pxl_col_in_fullres",
...
``` {r mito histogram}
hist(colData(spe_6471_mid)$subsets_mito_percent, breaks = 20)
```
...
qc_mito <- colData(spe_6471_mid)$subsets_mito_percent > 30
table(qc_mito)
colData(spe_6471_mid)$qc_mito <- qc_mito
plotQC(spe_6471_mid, type = "spots", 
       x_coord = "pxl_row_in_fullres",
       y_coord = "pxl_col_in_fullres",
       discard = "qc_mito")
...
hist(colData(spe_6471_mid)$cell_counts, breaks = 20)
```

### r distribution of cells per spot
```{r distribution of cells per spot}
tbl_cells_per_spot <- table(colData(spe_6471_mid)$cell_count)
```
...
qc_cell_count <- colData(spe_6471_mid)$cell_counts > 12
table(qc_cell_count)
colData(spe_6471_mid)$qc_cell_count <- qc_cell_count
...
library(scran)
set.seed(123)
qclus <- quickCluster(spe_6471_mid)
table(qclus)
```
...
spe_6471_mid <- computeSumFactors(spe_6471_mid, cluster = qclus)
summary(sizeFactors(spe_6471_mid))
```

### size factor hist 
```{r size factor hist }
hist(sizeFactors(spe_6471_mid), breaks = 20)
```


### calculate logcounts (log-transformed normalized counts)
```{r calculate logcounts (log-transformed normalized counts)}
spe_6471_mid <- logNormCounts(spe_6471_mid)
# check
assayNames(spe_6471_mid)
```
...
spe_6471_mid <- spe_6471_mid[!is_mito, ]
# fit mean-variance relationship
dec <- modelGeneVar(spe_6471_mid)
# select top HVGs
...
```{r compute PCA}
set.seed(123)
spe_6471_mid <- runPCA(spe_6471_mid, subset_row = top_hvgs)
reducedDimNames(spe_6471_mid)
dim(reducedDim(spe_6471_mid, "PCA"))
```
### UMAP
```{r UMAP}
library(uwot)
...
spe_6471_mid<- runUMAP(spe_6471_mid, dimred = "PCA")
reducedDimNames(spe_6471_mid)
dim(reducedDim(spe_6471_mid, "UMAP"))
colnames(reducedDim(spe_6471_mid, "UMAP")) <- paste0("UMAP", 1:2)
```

```{r plot top 2 PCA dimensions}
plotDimRed(spe_6471_mid, type = "PCA")
```
```{r plot top 2 UMAP dimensions}
plotDimRed(spe_6471_mid, type = "UMAP")
```
...
set.seed(123)
k <- 10
g <- buildSNNGraph(spe_6471_mid, k = k, use.dimred = "PCA")
g_walk <- igraph::cluster_walktrap(g)
clus <- g_walk$membership
table(clus)

# store cluster labels in column 'label' in colData
colLabels(spe_6471_mid) <- factor(clus)
```

```{r plot labels}
plotSpots(spe_6471_mid, annotate = 'label',
          x_coord = 'pxl_row_in_fullres',
          y_coord = 'pxl_col_in_fullres',
...
rownames(spe_6471_mid) <- rowData(spe_6471_mid)$symbols
# test for marker genes
markers <- findMarkers(spe_6471_mid, test = "binom", direction = "up")
